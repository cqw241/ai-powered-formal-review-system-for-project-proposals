...
开发阶段先用云端 API。Qwen3.8 的视觉能力直接作为可用能力使用，不另设模型能力验证任务；应用完成后再考虑本地 vLLM 部署。

本文确定产品方向和必要行为。框架、接口、表结构、Prompt、任务编排与参数随实现调整，不把设计样例当成冻结合同。当前仓库只有方案和开发样例，尚无可运行应用。
...
“项目总经费”和“申请经费”分别提取，按字段含义比较。暂时可以使用手工配置的一条规则，政策自动提取随后接入，不必等待整套规则平台。
...
先采用一个 Web 应用、一个后端和必要的后台任务处理。政策、文档、模型调用、规则计算和结果管理保持清楚的模块边界即可，不预先拆成独立微服务。

数据库保存政策、材料索引、审查结果和人工修改；文件放后端管理的存储目录或对象存储。数据库与文件存储的具体选型由实现决定。

Pi / pi-ai 可用于模型和工具调用，直接 SDK 也可先完成链路。业务数据不依赖特定运行时类型，不预设逐规则 Agent、第二验证模型、多档模型路由或专用遥测平台。
...
Prompt 随实际输入和返回结构编写，需要什么字段就提取什么字段。对缺失或无法读取的值保留原因，遇到具体问题再调整提示词或读取路径，不预写多套长模板及 A/B 评测计划。

请求失败时显示可理解的错误并允许重试；保留排查所需的请求标识和错误摘要即可。性能优化围绕已经影响使用的等待或重复调用处理，不预设耗时、成本和并发指标。
...
模型位置与前端高亮使用同一页面方向和尺寸；缩放、旋转由页面显示层统一处理。接入时检查返回字段、页码、坐标范围及高亮是否对齐，这是应用联调，不单独建设视觉能力评测集。

原文读取清楚时直接用于核对；遇到缺页、模糊或字段含义不清时显示具体待确认项，必要时重读局部区域。不要求每个字段都经过裁剪回读、异源交叉验证或独立验证器。
...
金额、日期、合计、相等与附件条件由程序处理。金额比较前统一单位，未知值不当作零；程序只执行已支持的计算，不执行模型生成的任意代码。算子按功能需要增加，不提前实现通用 DSL 全集。
...
现有输入位于 [开发材料索引](docs/kickoff/02_政策与材料索引.md)：1份模拟指南、3套模拟材料、10条规则和30个场景说明。它们可以直接帮助写功能，不是启动配额；其中受控变体尚需按当前功能制作，Markdown材料不能代替 PDF 页面联调。

按正在实现的功能选取相关例子，确认上传、提取、计算、定位、人工修改和重审能用。规则计算或状态处理出现问题时补对应检查，不要求每条规则固定测试数量，也不安排大规模标注、盲测、消融、采访或交叉计时。

比赛材料如实展示实际完成的功能与运行结果，不预填准确率、节省工时或获奖评分。

## 10. 开发顺序与交付

具体安排见 [八周行动计划](docs/competition-review/第一轮全面诊断与八周行动计划.md)。周次是工作顺序参考，按比赛实际日期与完成情况调整。
...
应用完成后，再根据比赛网络和部署需求考虑本地 vLLM。切换时复用现有模型接入层，本地部署不阻塞前述功能开发。
